...
public class TestMetaRegionReplicaReplicationEndpoint {
  ...
  public void testMetaRegionReplicaReplicationPeerIsCreated() throws IOException {
    String peerId = ServerRegionReplicaUtil.META_REGION_REPLICA_REPLICATION_PEER;
    ReplicationPeerConfig peerConfig = null;
    try {
      peerConfig = HTU.getAdmin().getReplicationPeerConfig(peerId);
    } catch (ReplicationPeerNotFoundException e) {
      LOG.warn("Region replica replication peer id=" + peerId + " not exist", e);
    }
    if (peerConfig != null) {
      HTU.getAdmin().removeReplicationPeer(peerId);
      peerConfig = null;
    }
    // Create the peer for meta table.
    ServerRegionReplicaUtil.setupRegionReplicaReplication(HTU.getConfiguration(),
